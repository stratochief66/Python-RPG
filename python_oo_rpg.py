...
def combat(enemy, prompt_text):
    ...
    if hero.health <= 0:
        prompt_text.add_to("You have died in glorious combat with a %s." % enemy.name)
        prompt_text.print_out()
        import sys
        sys.exit()

    if enemy.health <= 0:
        hero.xp += 10 * enemy.strength  # v2, step XP proportional to enemy strength
        prompt_text.add_to("You have killed the fierce %s and now have %s experience!" % (enemy.name, str(hero.xp)))
...
def prompt_decision(location, prompt_text):
    if location in wall_map:
        prompt_text.add_to("There must be a wall there. Lose a turn.")
        return hero.position
    elif location in objects_of_interest:
        if type(objects_of_interest[location]) == Monster:  # if you have bumped into a monster
            combat(objects_of_interest[location], prompt_text)
            if objects_of_interest[location].health > 0:
                prompt_text.add_to("Prepare to fight a %s!" % objects_of_interest[location].name)
                prompt_text.add_to("You have %s health remaining and the enemy has %s." % (hero.health, objects_of_interest[location].health))
            return hero.position
        else:
            prompt_text.add_to("You have found a %s and picked it up." % objects_of_interest[location].name)
            hero.inventory.append(objects_of_interest[location])
            objects_of_interest[location].position = ()
            return hero.position
    else:
        prompt_text.add_to("You moved.")
        return location
...
def user_interface(prompt_text): # requests a move command from the user, then prints all objects current positions
    import os
    import platform
    ...
    if input_char == 'N':
        hero.position = prompt_decision((hero.position[0] - 1, hero.position[1]), prompt_text)
    elif input_char == 'S':
        hero.position = prompt_decision((hero.position[0] + 1, hero.position[1]), prompt_text)
    elif input_char == 'E':
        hero.position = prompt_decision((hero.position[0], hero.position[1] + 1), prompt_text)
    elif input_char == 'W':
        hero.position = prompt_decision((hero.position[0], hero.position[1] - 1), prompt_text)
    elif input_char == 'Q':
